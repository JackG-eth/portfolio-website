...
const MobileNav: React.FC<{
	...
	return (
		<menu
			className={`fixed top-0 flex h-screen max-h-[100vh] min-h-[650px] w-screen max-w-[100vw] items-center justify-center overflow-hidden bg-portfolio-black transition-all duration-700 ease-in-out [transform:translate3d(0,0,0)] ${
				navOpen
					...
					<div className="flex h-full flex-grow flex-col items-start justify-center">
						{routes.map((route, i) => {
							return (
								<button
									key={i}
									className={`nav-link group relative text-[32px] text-white transition-colors duration-500 hover:text-portfolio-lightgreen ${
										i !== 0 ? "mt-6 md:mt-12" : ""
									} `}
									onClick={() => closeNav()}
								>
